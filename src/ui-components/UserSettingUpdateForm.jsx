...
export default function UserSettingUpdateForm(props) {
  const {
    id,
    userSetting,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...userSettingRecord };
    setLanguage(cleanValues.language);
    setTrackerAutoStart(cleanValues.trackerAutoStart);
    setTrackerStopOnNewStart(cleanValues.trackerStopOnNewStart);
    setTrackerViewMode(cleanValues.trackerViewMode);
    setTrackerViewSort(cleanValues.trackerViewSort);
    setDashboardTrackersPagination(cleanValues.dashboardTrackersPagination);
    setErrors({});
  };
  const [userSettingRecord, setUserSettingRecord] = React.useState(userSetting);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(UserSetting, id) : userSetting;
      setUserSettingRecord(record);
    };
    queryData();
  }, [id, userSetting]);
  React.useEffect(resetStateValues, [userSettingRecord]);
  const validations = {
    language: [],
    trackerAutoStart: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            UserSetting.copyOf(userSettingRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "UserSettingUpdateForm")}
    >
      <SelectField
        label="Language"
